...
export default function FoodUploadModal({
    ...
    const handleAnalyzePhoto = async () => {
        ...
        try {
            ...
            const data: FoodAnalysisResult = {
                food_name: selectedFile.name.replace(/\.[^.]+$/, "") || "食物照片",
                portion: "请确认份量",
                carbs_g: 35,
                protein_g: 18,
                fat_g: 12,
                calories: 35 * 4 + 18 * 4 + 12 * 9,
                confidence: 0.6,
                image_path: previewUrl || undefined,
                log_id: `photo-${userId}-${Date.now()}`,
            };
            setResult(data);
    ...
    const handleManualSubmit = async () => {
        ...
        try {
            if (useAIEstimate) {
                ...
                const ratio = manualWeight / 100;
                const data: FoodAnalysisResult = {
                    food_name: manualFoodName,
                    portion: `${manualWeight}g`,
                    carbs_g: Math.round(28 * ratio),
                    protein_g: Math.round(10 * ratio),
                    fat_g: Math.round(7 * ratio),
                    calories: Math.round((28 * 4 + 10 * 4 + 7 * 9) * ratio),
                    confidence: 0.7,
                    log_id: `manual-${userId}-${Date.now()}`,
                };
                setResult(data);
                setEditCarbs(data.carbs_g);
                setEditProtein(data.protein_g);
